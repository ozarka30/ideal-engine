#!/usr/bin/env python3
"""Writes docs/CONTENT_SCHEMA.md with verbatim excerpts from content/*.json."""
import json, re, os
...
def find(p, key, id_):
    ...
    raise KeyError(id_)
def js(o): return json.dumps(o, indent=2, ensure_ascii=False)
def excerpt(p, key, id_): return "```json\n" + js(find(p, key, id_)) + "\n```"
schema = json.load(open(os.path.join(ROOT, "schema", "content.schema.json")))
D = schema["$defs"]
def enum_of(name): return ", ".join("`%s`" % v for v in D[name]["enum"])
eff = D["Effect"]["properties"]
ON = ", ".join("`%s`" % v for v in eff["on"]["enum"])
DO = ", ".join("`%s`" % v for v in eff["do"]["enum"])
STATS = eff["stat"]["enum"]; FLAGS = eff["flag"]["enum"]
counts = {k: len(load(f)[k]) for f, k in [("employees.json","employees"),("rooms.json","rooms"),("furniture.json","furniture"),("recipes.json","recipes"),("riders.json","riders"),("modifiers.json","modifiers")]}
n_scripted = len([f for f in load("index.json")["files"] if f["def"] == "ScriptedRival"])
rc = load("recipes.json")["recipes"]
by_class = {c: sum(1 for r in rc if r["class"] == c) for c in ("promotion", "renovation", "ritual")}

doc = f"""# Company Wars — Content Schema
...
`stat` names one of: {", ".join("`%s`" % s for s in STATS)}.
...
{excerpt("employees.json", "employees", "emp.counsel")}

A Sales passive is an `economy` effect the sim never reads:

{excerpt("employees.json", "employees", "emp.telemarketer")}

A ritual result, with a `countsAsDept` and two immunities:

{excerpt("employees.json", "employees", "emp.x_salaryman_who_never_left")}
...
{excerpt("rooms.json", "rooms", "room.server_room")}
...
{excerpt("rooms.json", "rooms", "room.boardroom")}
...
{excerpt("furniture.json", "furniture", "furn.fax_machine")}

A subject-driven `afterFire` on furniture — "every fourth time a neighbour fires":

{excerpt("furniture.json", "furniture", "furn.copier")}
...
{excerpt("recipes.json", "recipes", "recipe.senior_dev")}

A wildcard input and a room-context result:

{excerpt("recipes.json", "recipes", "recipe.director")}

{excerpt("recipes.json", "recipes", "recipe.break_room_iii")}
...
{excerpt("riders.json", "riders", "rider.contractual_obligation")}

{excerpt("modifiers.json", "modifiers", "mod.lean")}

A rival-only gimmick is the same type with `rivalOnly: true` and a `teaches` line the
dossier shows:

{excerpt("modifiers.json", "modifiers", "mod.g_regulatory_capture")}
...
{excerpt("founders.json", "founders", "founder.the_founder")}
...
```json
{js(find("rivals/scripted/boss_regional_rival.json", "snapshot", None) if False else load("rivals/scripted/boss_regional_rival.json")["snapshot"]["floors"][1])}
```
...
| Type | Count | Notes |
| --- | --- | --- |
| Employees | {counts["employees"]} | 30 across five departments; 5 extraplanar in the shop; 5 ritual results |
| Rooms | {counts["rooms"]} | Reception is fixed; 3 executive, 2 extraplanar, 1 security |
...
"""
open(os.path.join(ROOT, "docs", "CONTENT_SCHEMA.md"), "w").write(doc)
print("wrote CONTENT_SCHEMA.md", len(doc.splitlines()), "lines")
