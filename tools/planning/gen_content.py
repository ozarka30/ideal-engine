...
rules = OD([
    ...
    ("time", OD([("ticksPerSecond", 20), ("quarterTicks", 1200),
                 ...
                 ("pushMult", [1000, 1400, 2000, 3000]),
                 ("regenMult", [1000, 600, 200, 0])])),
    ("goodwill", OD([("baseCap", OD([("constant", 600), ("perRound", 100)])),
                     ("regenBasePermille", 30), ("regenInterval", 40),
                     ("regenSuppressWindow", 20), ("suppressThresholdPermille", 40)])),
    ("morale", OD([("interval", 20), ("perStack", 8), ("ratePermille", 500)])),
    ("anomaly", OD([("selfCostPermille", 250)])),
    ("share", OD([("total", 10000), ("start", 5000),
                  ("spPerPushPermille", [8000, 7000, 6000, 5200, 4500, 3900, 3400, 3000,
                                         2600, 2300, 2000, 1750, 1500, 1300, 1150, 1000])])),
    ...
    ("upkeep", OD([("goodwillPerUnpaidBudget", 100)])),
])
...
statuses = [
    OD([("id", "status.burnout"), ("name", "Burnout"), ("maxStacks", 5), ("durationTicks", None),
        ...
        ("pushPenaltyPermillePerStack", 50), ("moralePerStackPerEvent", 8),
        ("onExpire", []), ("tone", "anomalous"),
        ("text", "Stacking. Each second the firm takes Morale per stack; the employee's output falls 5% per stack. Never expires in a fight.")]),
    OD([("id", "status.overtime"), ("name", "Overtime"), ("maxStacks", 2), ("durationTicks", 60),
        ("expires", True), ("cooldownRatePermillePerStack", 500),
        ("pushPenaltyPermillePerStack", 0), ("moralePerStackPerEvent", 0),
...
def push(v, **kw): return eff("ability", "push", value=v, target=firm("enemy"), **kw)
def anomaly(v, **kw): return eff("ability", "anomaly", value=v, target=firm("enemy"), **kw)
def restore(v, **kw): return eff("ability", "restore", value=v, target=firm("own"), **kw)
...
# Engineering
emp("emp.intern", "Intern", ENG, 1, 2, 60, [push(35)], tags=("junior",),
    flavor="Unpaid, in the sense that the invoice is emotional.")
emp("emp.junior_dev", "Junior Developer", ENG, 1, 3, 80, [push(60, name="Ship Feature")], tags=("junior",),
    flavor="Ships it. Whether it works is a Month 2 problem.")
emp("emp.qa_tester", "QA Tester", ENG, 1, 3, 40, [push(25, name="Bug Report")],
    flavor="Small, frequent, and never quite enough to register.")
emp("emp.senior_dev", "Senior Developer", ENG, 2, 6, 80, [push(150, name="Ship Feature")],
    cdByMonth=OD([("2", 60)]), flavor="Accelerates during Crunch. This is not a compliment.")
emp("emp.devops", "DevOps", ENG, 2, 6, 120,
    [push(90, name="Deploy"), after(status(OVT, 1, own("adjacent", dept=ENG)))],
    flavor="Every deploy is someone else's overtime.")
emp("emp.sysadmin", "Sysadmin", ENG, 2, 6, 100, [push(100, name="Reboot"), flag("bureaucracyImmune", SELF)],
    flavor="Cannot be slowed. Has never once been in a meeting.")
emp("emp.architect", "Architect", ENG, 3, 10, 160,
    [push(400, name="Refactor"), after(status(OVT, 1, own("adjacent", dept=ENG)), everyN=3)],
    flavor="Every third Refactor is the one that makes everyone stay late.")
emp("emp.cto", "CTO", ENG, 3, 10, 200,
    [push(250, name="Roadmap"), stat("cooldown", subj("sameFloor", dept=ENG), permille=900)],
...
emp("emp.paralegal", "Paralegal", LEG, 1, 3, 60,
    [status(BUR, 1, MP_LO, name="Paperwork"), stat("goodwillCap", SELF, amount=100)],
    flavor="Generates forms. The forms are the weapon.")
emp("emp.compliance_officer", "Compliance Officer", LEG, 1, 3, 80,
    [cleanse(BUR, 1, own("adjacent"), name="Waiver"), stat("goodwillCap", SELF, amount=150)],
    flavor="Unblocks the people next to them. Files a report about it.")
emp("emp.counsel", "Counsel", LEG, 2, 6, 120,
    [push(90, name="Cease & Desist"), after(status(BUR, 2, HI_HI)),
     stat("goodwillCap", SELF, amount=250), stat("regenPerEvent", SELF, amount=40)],
    flavor="Sends a letter. The letter has consequences.")
emp("emp.patent_attorney", "Patent Attorney", LEG, 2, 6, 140,
    [push(130, name="Litigation"), stat("goodwillCap", SELF, amount=150)],
...
emp("emp.general_counsel", "General Counsel", LEG, 3, 10, 200,
    [status(FRZ, 1, HI_HI, name="Injunction", durationTicks=60),
     stat("goodwillCap", SELF, amount=500), stat("regenPerEvent", SELF, amount=80)],
    flavor="Freezes their best person for three seconds. Bills for six.")

# HR
emp("emp.recruiter", "Recruiter", HR, 1, 3, 100,
    [restore(80, name="Team Building"), stat("regenPerEvent", SELF, amount=30)],
    flavor="Restores Goodwill. Has a lanyard.")
emp("emp.office_manager", "Office Manager", HR, 1, 3, 120,
    [cleanse(BRN, 1, own("adjacent"), name="Snack Run"), stat("goodwillCap", SELF, amount=50)],
    flavor="Removes one Burnout from each neighbour. Knows where the good stapler is.")
emp("emp.hr_manager", "HR Manager", HR, 2, 6, 120,
    [restore(150, name="Wellness Program"), after(cleanse(BRN, 2, own("sameFloor")))],
...
emp("emp.head_of_people", "Head of People", HR, 3, 10, 160,
    [restore(300, name="Retention Bonus"), stat("burnoutMaxOverride", subj("all"), amount=3)],
...
emp("emp.telemarketer", "Telemarketer", SAL, 1, 2, 40,
    [push(20, name="Cold Call"), eff("economy", "stat", stat="income", subject=SELF, amount=1)],
...
emp("emp.sales_rep", "Sales Rep", SAL, 1, 3, 60,
    [push(40, name="Cold Call"), eff("economy", "stat", stat="income", subject=SELF, amount=1)],
    flavor="Grows the budget between fights and chips at Goodwill during them.")
emp("emp.account_manager", "Account Manager", SAL, 2, 6, 100,
    [push(110, name="Close Deal"), eff("economy", "stat", stat="income", subject=SELF, amount=2)],
    flavor="Closes. Expenses lunch.")
emp("emp.headhunter", "Headhunter", SAL, 2, 6, 140,
    [status(BRN, 2, HI_HI, name="Poach"), eff("economy", "stat", stat="income", subject=SELF, amount=1)],
    flavor="Burns out their best person by offering them a job. Morale damage does not appear on a balance sheet.")
emp("emp.sales_director", "Sales Director", SAL, 3, 10, 240,
    [push(OD([("base", 300), ("perTag", "sales"), ("each", 50)]), name="Quarterly Target"),
...
emp("emp.key_account_manager", "Key Account Manager", SAL, 3, 10, 160,
    [push(200, name="Enterprise Deal"), after(restore(100)),
...
emp("emp.x_salaryman_ghost", "Salaryman Ghost", XP, 2, 5, 60,
    [anomaly(90, name="Overtime Eternal"), flag("overtimePermanent", SELF)], extra=True,
...
emp("emp.x_office_lady", "Office Lady of the Third Floor", XP, 2, 5, 120,
    [anomaly(60, name="Filing"), after(status(BUR, 1, enemy("same_floor_index", "lowest_cooldown_remaining")))],
...
emp("emp.x_auditor", "The Auditor", XP, 3, 5, 240,
    [anomaly(OD([("permilleOfTargetCap", 150)]), name="Audit"), flag("cannotBeRetriggered", SELF)],
    extra=True, flavor="Deals 15% of the rival's Goodwill cap, straight to the bar. Cannot be hurried.")
emp("emp.x_fax_spirit", "Fax Spirit", XP, 2, 5, 100,
    [anomaly(40, name="Transmission"), after(retrig(own("adjacent", dept=LEG, pick="highest_base_value")))],
    extra=True, flavor="A fax machine that died and kept working.")
emp("emp.x_kappa_intern", "Kappa Intern", XP, 1, 5, 40,
    [anomaly(30, name="Splash"), stat("regenPerEvent", SELF, amount=40)], extra=True,
...
emp("emp.x_salaryman_who_never_left", "Salaryman Who Never Left", XP, 3, 0, 80,
    [anomaly(200, name="Loyalty"), flag("bureaucracyImmune", SELF), flag("frozenImmune", SELF)],
...
emp("emp.x_the_chairman", "The Chairman", XP, 3, 0, 300,
    [anomaly(600, name="Board Resolution"), stat("goodwillCap", SELF, amount=-300)],
    extra=True, inShop=False, tags=("ritual",),
    flavor="Six hundred Anomaly every fifteen seconds. The firm's reputation pays for the chair.")
emp("emp.x_the_partner", "The Partner", XP, 3, 0, 160,
    [anomaly(250, name="Equity"), stat("goodwillCap", SELF, amount=-200)],
...
emp("emp.x_efficiency_wraith", "Efficiency Wraith", XP, 2, 0, 100,
    [anomaly(50, name="Review"), after(status(BRN, 1, enemy("most_populated_floor", "all")))],
...
emp("emp.x_recruiting_oni", "Recruiting Oni", XP, 2, 0, 140,
    [anomaly(80, name="Offer"), after(status(BRN, 2, HI_HI))],
...
room("room.reception", "Reception", 2, 2, ["floor.g"], 0, [
    stat("goodwillCap", OCC, amount=100),
    stat("push", OCC, permille=800),
    stat("regenPerEvent", OCC, amount=30, fromTier=3),
], kind="reception", fixed=True, flavor="Where reputation lives. Every person at the desk is +100 Goodwill.")
room("room.open_plan", "Open Plan Office", 2, 3, ["floor.f1", "floor.f2"], 13, [
    stat("push", subj("occupants", dept=ENG), permille=1200),
...
room("room.server_room", "Server Room", 2, 2, ["floor.f1", "floor.f2"], 9, [
    stat("push", subj("occupants", dept=ENG), permille=1350),
...
], flavor="Legal passives ×1.5 and every Bureaucracy they apply gains a stack. Tier III: their Goodwill cannot be eroded.")
room("room.sales_floor", "Sales Floor", 2, 2, ["floor.f1", "floor.f2"], 9, [
    stat("push", subj("occupants", dept=SAL), permille=1200),
...
room("room.break_room", "Break Room", 1, 2, ["floor.g", "floor.f1", "floor.f2", "floor.f3"], 5, [
    stat("push", subj("occupants", notDept=HR), permille=500),
    flag("burnoutImmune", subj("occupants", notDept=HR)),
    stat("restore", subj("occupants", dept=HR), permille=1500),
    cleanse(BRN, 1, own("adjacent"), on="periodic", every=200, subject=OCC, fromTier=3),
], flavor="Non-HR occupants do half the work and cannot burn out. HR restores half again as much.")
...
room("room.corner_office", "Corner Office", 1, 2, ["floor.f3"], 5, [
    stat("push", OCC, permille=1600),
    stat("goodwillCap", OCC, amount=-100, untilTier=3),
], kind="executive", maxOcc=1, flavor="One person, ×1.6, and −100 Goodwill for the ego until Tier III.")
room("room.summoning_circle", "Summoning Circle", 2, 2, ["floor.b1"], 9, [
    stat("anomalySelfCost", subj("occupants", dept=XP), permille=-125),
    status(BRN, 1, HI_HI, on="afterFire", everyN=1, subject=subj("occupants", dept=XP), fromTier=3),
], kind="extraplanar", flavor="Halves the Anomaly self-cost. Required by every Ritual.")
room("room.meeting_room", "Meeting Room", 2, 2, ["floor.f1", "floor.f2"], 9, [
    stat("push", OCC, permille=1100),
...
room("room.executive_lounge", "Executive Lounge", 2, 2, ["floor.f3"], 9, [
    stat("goodwillCap", OCC, amount=150),
    stat("push", OCC, permille=800),
    stat("regenPerEvent", OCC, amount=20, fromTier=3),
], kind="executive", flavor="The turtle's executive floor. +150 Goodwill per occupant, ×0.8 output.")
room("room.archive", "Archive", 1, 2, ["floor.b1"], 5, [
    stat("goodwillCap", OCC, amount=150),
    stat("anomaly", OCC, permille=800),
...
def furn(id, name, w, h, cost, effects, wall=False, floors_=None, rarity="common", flavor=""):
    ...
    F.append(d)
ADJ = subj("adjacent")
furn("furn.whiteboard", "Whiteboard", 1, 1, 2, [stat("flatPush", subj("adjacent", dept=ENG), amount=15)],
     wall=True, flavor="+15 Push to adjacent Engineering. Recipe input.")
...
furn("furn.filing_cabinet", "Filing Cabinet", 1, 1, 2, [stat("goodwillCap", subj("adjacent", dept=LEG), amount=60)],
     flavor="+60 Goodwill per adjacent Legal. Recipe input.")
...
furn("furn.ofuda", "Ofuda", 1, 1, 2, [stat("anomalySelfCost", subj("adjacent", dept=XP), permille=-125)],
     wall=True, floors_=["floor.b1"], flavor="Halves the Anomaly self-cost of adjacent extraplanar staff. Stacks with the Circle to zero.")
furn("furn.desk_phone", "Desk Phone", 1, 1, 2, [stat("flatPush", subj("adjacent", dept=SAL), amount=10)],
     flavor="+10 Push to adjacent Sales.")
...
furn("furn.potted_plant", "Potted Plant", 1, 1, 2, [stat("goodwillCap", ADJ, amount=30)],
     flavor="+30 Goodwill per neighbour. Dies mysteriously in round 3 of the first run.")
...
rider("rider.poor_reception", "Poor Reception", "Reception grants no Goodwill while employed.", [flag("receptionDisabled", subj("firm"))])
...
rider("rider.contractual_obligation", "Contractual Obligation", "At the Bell, the firm takes 200 Morale.", [eff("banner", "morale", month=3, value=200, target=firm("own"))])
...
mod("mod.lean", "Lean", "Goodwill cap −200; income +¥2 per round.",
    [stat("goodwillCap", subj("firm"), amount=-200), eff("economy", "stat", stat="income", subject=subj("firm"), amount=2)],
    board=OD([("cost", "−200 Goodwill cap"), ("benefit", "+¥2 income per round")]))
...
mod("mod.hostile_environment", "Hostile Environment", "All output ×1.1; nobody can burn past 3 stacks — because they leave.",
    [stat("push", ALL, permille=1100), stat("burnoutMaxOverride", ALL, amount=3)],
...
# rival-only gimmicks
mod("mod.g_deep_pockets", "Deep Pockets", "Goodwill cap ×1.5.", [stat("goodwillCapMult", subj("firm"), permille=1500)], rivalOnly=True, teaches="Push alone is slow; bring Burnout.")
...
mod("mod.g_regulatory_capture", "Regulatory Capture", "Regen is never suppressed.", [flag("regenNeverSuppressed", subj("firm"))], rivalOnly=True, teaches="The Act 2 boss's signature. Unwinnable without Morale or Anomaly.")
...
FOUNDERS_BY_ARCH = {"generalist": ["founder.sato", "founder.kitamura"], "turtle": ["founder.okada", "founder.sato"],
                    "burst": ["founder.nakagawa", "founder.kitamura"], "economy": ["founder.hoshino", "founder.kitamura"],
                    "burnout": ["founder.moriyama", "founder.ueda"], "management": ["founder.moriyama", "founder.the_founder"]}
...
templates = [
    T("rival.t_generalist", "generalist", ["Kobayashi Holdings", "Sato & Sons", "Nakamura Trading", "Fujiwara Logistics"],
      ...
      shoplist(("emp.junior_dev", 4, 1), ("emp.sales_rep", 3, 1), ("emp.paralegal", 2, 1), ("emp.qa_tester", 2, 1), ("emp.senior_dev", 3, 4), ("emp.counsel", 1, 6), ("emp.team_lead", 2, 3), ("emp.account_manager", 2, 5), ("emp.architect", 1, 9)),
      ["mod.g_zaibatsu", "mod.g_skeleton_crew"], LAY(["floor.f1", "floor.f2", "floor.g"], ["engineering", "sales", "legal", "management"])),
    T("rival.t_turtle", "turtle", ["Compliance Partners", "Harada Legal", "Mizuno Assurance", "Ishikawa & Ishikawa"],
      shoplist(("room.legal_dept", 4, 2), ("room.executive_lounge", 2, 7), ("room.kitchenette", 1, 3), ("room.security_desk", 1, 4)),
      shoplist(("emp.paralegal", 5, 1), ("emp.recruiter", 3, 1), ("emp.compliance_officer", 2, 2), ("emp.counsel", 3, 4), ("emp.patent_attorney", 1, 5), ("emp.hr_manager", 2, 5), ("emp.general_counsel", 1, 9), ("emp.head_of_people", 1, 10)),
      ["mod.g_deep_pockets", "mod.g_regulatory_capture"], LAY(["floor.f1", "floor.g", "floor.f2"], ["legal", "hr", "sales"])),
    T("rival.t_burst", "burst", ["Tanaka Systems", "Ono Engineering", "Kurokawa Labs", "Yamashita Digital"],
      shoplist(("room.server_room", 4, 2), ("room.open_plan", 2, 2), ("room.meeting_room", 1, 5), ("room.corner_office", 1, 8)),
      shoplist(("emp.junior_dev", 4, 1), ("emp.intern", 2, 1), ("emp.senior_dev", 4, 4), ("emp.devops", 2, 5), ("emp.team_lead", 3, 2), ("emp.architect", 3, 9), ("emp.director", 1, 11), ("emp.cto", 1, 12)),
      ["mod.g_skeleton_crew", "mod.g_night_shift"], LAY(["floor.f1", "floor.f2", "floor.f3"], ["engineering", "management"])),
    T("rival.t_economy", "economy", ["Suzuki Sales Co.", "Matsumoto Direct", "Ando Telemarketing", "Hayashi Retail"],
      shoplist(("room.sales_floor", 4, 2), ("room.mail_room", 1, 2), ("room.open_plan", 1, 5), ("room.boardroom", 1, 9)),
      shoplist(("emp.sales_rep", 5, 1), ("emp.telemarketer", 3, 1), ("emp.account_manager", 4, 4), ("emp.headhunter", 1, 6), ("emp.key_account_manager", 1, 9), ("emp.sales_director", 2, 10), ("emp.middle_manager", 1, 7)),
      ["mod.g_zaibatsu", "mod.g_deep_pockets"], LAY(["floor.f1", "floor.f2", "floor.g"], ["sales", "management", "engineering"]), budget=1050),
    T("rival.t_burnout", "burnout", ["Watanabe Consulting", "Kimura Advisory", "Nishimura Group", "The Yoshida Practice"],
    ...
    T("rival.t_management", "management", ["Ito Corporation", "Shimizu Enterprises", "Goto Industrial", "Maeda Holdings"],
      ...
      shoplist(("emp.team_lead", 5, 1), ("emp.junior_dev", 3, 1), ("emp.middle_manager", 3, 4), ("emp.senior_dev", 3, 4), ("emp.director", 2, 9), ("emp.vp_operations", 1, 11), ("emp.architect", 1, 10)),
...
balance = OD([
    ...
    ("populations", OD([
        ...
        ("strongest_defence", OD([("description", "A hill-climb over legal, constructible, gimmick-free builds at the round, maximising the tick at which Goodwill first reaches 0 against median_attacker; 200 iterations from the turtle template at 1200 permille"), ("search", "maximize_break_tick"), ("start", "rival.t_turtle"), ("budgetPermille", 1200), ("iterations", 200)])),
    ...
    ("bands", OD([
        ("archetypeVsField", [420, 580]),
        ("counterPair", [580, 750]),
        ("mirror", [470, 530]),
        ("fightMedianEndTick", [700, 1000]),
        ("bellRateMax", 250), ("drawRateMax", 20),
        ("firstShareMoveMedianTick", 200), ("firstShareMoveP90Tick", 400),
    ...
    ("counters", [
        OD([("winner", "turtle"), ("loser", "burst"), ("why", "Legal cap and regen outlast a build whose damage arrives in bursts")]),
        OD([("winner", "burst"), ("loser", "economy"), ("why", "Economy scales into rounds it does not survive")]),
        OD([("winner", "burnout"), ("loser", "turtle"), ("why", "Morale ignores Goodwill and erodes the cap the turtle paid for")]),
        OD([("winner", "management"), ("loser", "burnout"), ("why", "Retriggers with HR cleanse out-tempo a slow Burnout stack")]),
        OD([("winner", "economy"), ("loser", "management"), ("why", "Chip that never stops suppressing regen, plus income that outgrows a retrigger core")]),
    ...
    ("bossCounters", [
        ...
        OD([("boss", "rival.boss_compliance_office"), ("favoured", "burnout"), ("favouredMin", 600), ("punished", "burst"), ("punishedMax", 300)]),
        OD([("boss", "rival.boss_parent_company"), ("favoured", "burnout"), ("favouredMin", 450), ("punished", "burst"), ("punishedMax", 300)]),
    ]),
    ("invariants", [
        inv("inv.break_guaranteed", "No defence survives to the Bell", "Against median_attacker, the strongest_defence build's Goodwill reaches 0 before the Bell in every seed.",
            "strongest_defence vs median_attacker", "max over seeds of first tick at which defender goodwill == 0", "<", 1160, "smoke+nightly", "fail", "DESIGN_BRIEF risk 3; D-05"),
        inv("inv.bar_moves_early", "Fights do not open flat", "In field matches the Market Share bar first moves early.",
            "field vs field", "median and P90 over matches of the first tick with shareDelta != 0", "median<=200,p90<=400", [200, 400], "smoke+nightly", "fail", "DESIGN_BRIEF risk 4; PLANNING_PROMPT"),
        inv("inv.archetype_band", "No archetype dominates the field", "Each archetype's win rate against the field, mirror excluded, lies inside the band.",
            "each archetype vs field", "win rate permille", "in", [420, 580], "smoke+nightly", "fail", "PLANNING_PROMPT invariant 3"),
        ...
        inv("inv.fight_length", "Fights end in the window", "Median fight length is 35–50 s, the Bell resolves few fights, draws are rare.",
            "field vs field", "median endTick; bell-resolution rate; draw rate", "median in [700,1000]; bell<=250; draw<=20", [[700, 1000], 250, 20], "smoke+nightly", "fail", "D-21; GAME_DESIGN §11"),
        inv("inv.chip_cannot_suppress", "Chip cannot hold regen down alone", "No shop-available employee whose base value reaches the round's suppression threshold has a cooldown under 60 ticks.",
            "static: employees x rounds offered", "for each (employee, round): baseValue >= suppressThreshold(round) implies cooldownTicks >= 60", "all", True, "commit", "fail", "D-04 trade-off"),
        inv("inv.single_hit_cap", "No one hit claims the bar", "No single resolution in a field match moves Market Share by more than the cap.",
            "field vs field", "max over entries of |shareDelta|", "<=", 1500, "smoke+nightly", "fail", "D-06 trade-off"),
        ...
        inv("inv.ability_diversity", "Wins are not one card", "In field wins, no single ability supplies most of the winner's Push.",
            "field vs field, winners", "P50 over matches of max share permille of totalPush from one abilityId", "<=", 500, "nightly", "fail", "PLANNING_PROMPT quality bar: combination over quantity"),
...
tutorial = OD([("id", "tutorial.first_run"), ("hints", [
    ...
    OD([("round", 7), ("text", "That was a lawyer. Legal makes Goodwill hard to break. There is a way around a balance sheet.")]),
...
], archetype="economy", founder="founder.hoshino", note="Fight 5. An economy build. Chip damage and regen; the player learns severance this round."))
...
], archetype="turtle", founder="founder.okada", note="Fight 7. The first Counsel the player meets. Goodwill that will not break in Month 1."))
...
], modifiers_=["mod.g_regulatory_capture"], gimmick="mod.g_regulatory_capture", archetype="turtle", exempt=True, founder="founder.okada",
   note="Act 2 boss. Regen is never suppressed; two Legal Departments at Tier I and II; a Reception full of Paralegals at Tier III. Push alone cannot beat it before the Bell. Bring Morale or Anomaly."))
